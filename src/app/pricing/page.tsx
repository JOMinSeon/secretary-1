        ...
        <div className="py-12 space-y-12">
            <div className="text-center space-y-4">
                <h1 className="text-4xl font-extrabold text-slate-900 tracking-tight sm:text-5xl">
                    비즈니스에 딱 맞는 <span className="text-indigo-600">성장 플랜</span>을 선택하세요
                </h1>
                <p className="text-xl text-slate-500 max-w-2xl mx-auto">
                    axAI Secretary와 함께 영수증 관리 시간을 90% 단축하고 환급액을 극대화하세요.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto px-4">
                {plans.map((plan, i) => (
                    ...
                    >
                        <Card className={`relative h-full border-2 flex flex-col ${plan.popular ? 'border-indigo-600 shadow-2xl scale-105 z-10' : 'border-slate-100 shadow-sm'}`}>
                            {plan.popular && (
                            ...
                            <CardHeader>
                                <div className={`${plan.color} w-12 h-12 rounded-2xl flex items-center justify-center mb-4 shadow-lg shadow-indigo-100 text-white`}>
                                    <plan.icon size={24} />
                                </div>
            ...
            <div className="max-w-3xl mx-auto bg-white p-8 rounded-3xl border border-dashed border-slate-200 text-center space-y-4">
                <h3 className="text-xl font-bold text-slate-900">기업용 커스텀 플랜이 필요하신가요?</h3>
                <p className="text-slate-500">대규모 인원 관리가 필요한 가맹본부 또는 기업을 위한 맞춤형 솔루션을 제안해드립니다.</p>
                <Button variant="link" className="text-indigo-600 font-bold hover:no-underline">
                    영업팀에 문의하기 <ArrowRight className="w-4 h-4 ml-1" />
                </Button>
            </div>
        </div>
    );
